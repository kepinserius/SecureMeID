...
class DocumentService extends ChangeNotifier {
  ...
  Future<Document?> getDocument(String id) async {
    ...
    try {
      ...
      final cached = _documents.firstWhere((doc) => doc.id == id, orElse: () => Document.empty());
  ...
  Future<bool> deleteDocument(String id) async {
    ...
    try {
      // Panggil fungsi di canister untuk menghapus dokumen
      // Placeholder untuk implementasi masa depan
      
      // Hapus dari cache jika sukses
  ...
  Future<Map<String, dynamic>> generateVerificationToken(
    ...
    try {
      return await _icpService.generateVerificationToken(documentId, fields, expirySeconds);
    } catch (e) {
      ...
      final expiryTime = DateTime.now().add(Duration(seconds: expirySeconds)).millisecondsSinceEpoch;
  ...
  Future<Map<String, dynamic>> verifyToken(String token) async {
    try {
      final result = await _icpService.verifyToken(token);
      return result;
    } catch (e) {
  ...
  Future<void> _loadDocumentsFromDisk() async {
    try {
      if (_documentsBox.containsKey('documentsList')) {
        ...
        _documents = jsonList.map((jsonItem) => Document.fromJson(jsonItem)).toList();
      }
  ...
  Future<File> getDocumentFile(String documentId, String pin, String localFileName) async {
    try {
  ...
  Future<void> syncWithBlockchain(String userWalletAddress) async {
    try {
      ...
      final blockchainDocuments = await _blockchainService.getAllDocuments(userWalletAddress);
      ...
      for (final blockchainDoc in blockchainDocuments) {
        ...
        final existingIndex = _documents.indexWhere((doc) => doc.hash == documentHash);
        ...
        if (existingIndex == -1) {
          ...
          _documents.add(Document(
            ...
            ipfsCid: ipfsCid,
            owner: userWalletAddress,
            createdAt: DateTime.fromMillisecondsSinceEpoch(blockchainDoc['timestamp']),
            updatedAt: DateTime.fromMillisecondsSinceEpoch(blockchainDoc['timestamp']),
            isVerified: isVerified,
            verifiedBy: blockchainDoc['verifier'],
            verifiedAt: blockchainDoc['verificationTimestamp'] != null
                ? DateTime.fromMillisecondsSinceEpoch(blockchainDoc['verificationTimestamp'])
                : null,
            metadata: {},
            txHash: '',
          ));
