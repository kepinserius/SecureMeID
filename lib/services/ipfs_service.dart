...
class IPFSService extends ChangeNotifier {
  ...
  Future<String> uploadFile(File file, {String? encryptionKey}) async {
    try {
      ...
      if (encryptionKey != null) {
        ...
        content = _encryptionService.encrypt(base64Content, encryptionKey);
      } else {
  ...
  Future<String> uploadString(String data, {String? encryptionKey}) async {
    try {
      ...
      final content = encryptionKey != null 
          ? _encryptionService.encrypt(data, encryptionKey)
          : data;
  ...
  Future<Uint8List> downloadFile(String cid, {String? decryptionKey}) async {
    try {
      ...
      if (decryptionKey != null) {
        dataString = _encryptionService.decrypt(content, decryptionKey);
      } else {
        dataString = content;
      }
  ...
  Future<String> downloadString(String cid, {String? decryptionKey}) async {
    try {
      ...
      // Decrypt content if decryption key is provided
      if (decryptionKey != null) {
        return _encryptionService.decrypt(content, decryptionKey);
      } else {
        return content;
      }
    } catch (e) {
  ...
  Future<File> saveFileToLocal(String cid, String filename, {String? decryptionKey}) async {
    try {
      // Download file data
  ...
  Future<String> _realUploadToIPFS(Uint8List bytes) async {
    try {
      final request = http.MultipartRequest('POST', Uri.parse('$_ipfsApiUrl/add'));
